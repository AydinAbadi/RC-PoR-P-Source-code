  ...
  for (int  k = 0; k < number_of_levels; k++){
    ...
    for(int i = 0; i < temp_size_2;){
      if(k == 0){
        ...
        mpz_init_set(nodes[k][j], res_[0]);
        ///////////
        //string s_val_ = mpz_get_str(NULL, 10, res_[0]);
        //map.insert(make_pair(s_val_, j));
        ///////////////////
        //insert res_[0] and j to the map.


        //insert res_[0] and j in to the map
...
//===============================
bigint** build_MT_tree_(bigint* file, int file_size, unordered_map <string, int>*& map){
  ...
  for (int  k = 0; k < number_of_levels; k++){
    ...
    for(int i = 0; i < temp_size_2;){
      if(k == 0){
        ...
        ///////////
        string s_val_ = mpz_get_str(NULL, 10, res_[0]);
        //map[k].insert(make_pair(s_val_, j));
        //map[k][s_val_]=j;
        //cout<<"\n-----"<<endl;
        //cout<<"\n j:"<<j<<endl;
        //cout<<"\n in build_tree_--map["<<k<<"]["<<s_val_<<"]:"<<map[k][s_val_]<<endl;
        //cout<<"\n-----"<<endl;
        ////////
        mpz_clear(res_[0]);
        j++;
        ...
        //map[k].insert(make_pair(s_val_, j));
      //  map[k][s_val_]=j;
        //cout<<"\n in gen proof--map["<<s_val_<<"]:"<<map[s_val_]<<endl;
        // cout<<"\n-----"<<endl;
        // cout<<"\n j:"<<j<<endl;
        // cout<<"\n in build_tree_--map["<<k<<"]["<<s_val_<<"]:"<<map[k][s_val_]<<endl;
        // cout<<"\n-----"<<endl;
...
//=======
int find_index_2(bigint* set, int set_size, bigint val, bool &res_,int offset){
...
// }
//================
// int find_index_(unordered_map <string, int> map, bigint val, bool &res_){
//
//   string str_val = mpz_get_str(NULL, 10, val);
//   //cout<<"\n in find_index_"<<endl;
//   //if (map.find(str_val) != map.end()){
//     res_ = true;
//     //return map.bucket(str_val);
//     return map[str_val];
//    //}
//   //res_ = false;
//   //cout<<"\n in find_index_--map["<<str_val<<"]:"<<map[str_val]<<endl;
//   //return 0;
//
//
// }
  ...
  for(int i = 0; i < number_of_chall; i++){
    ...
    for (int  k = 0; k < number_of_levels; k++){
      ...
      if(k == 0){
        ...
        int index = find_index_2(nodes[k-1], nonil, temp_hash[0], res_, offset);
        offset = sqrt(index);//xxxx

        // cout<<"\n-------"<<endl;
        // //cout<<"\n---res:"<<res_<<" -----"<<endl;
        // cout<<"\n in gen_proof--index_:"<<index_<<endl;
        // int index = find_index_(map[k-1], temp_hash[0], res_);
         //cout<<"\n in gen_proof--index:"<<index<<endl;
        //cout<<"\n-------"<<endl;
        if(index % 2 == 0 && res_){
...
int main() {
  ...
  bigint** nodes, **nodes_;
  int file_size = 67108864;//2^20= 1048576
  ...
  int map_size = log2(file_size);
  map = new unordered_map <string, int>[map_size];
  ...
  double start_proof = clock();

  bigint*** proof_ = gen_proof(number_of_chall, chall, encoded_file, file_size, nodes);
  double end_proof = clock();//xxxxx
  double proof_time = end_proof-start_proof;
  float proof_time_ = proof_time / (double) CLOCKS_PER_SEC;
  cout<<"\n proof_time: "<<proof_time_<<endl;//***
